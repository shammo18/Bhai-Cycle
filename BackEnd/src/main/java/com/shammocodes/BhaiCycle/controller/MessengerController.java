...
public class MessengerController {
    ...
    private String doMessageAction(String action ,String actionId , String senderId)
    {
        ...
        {
            return authService.setUserActive(actionId , senderId);
        }else if (action.equals("accept")) {
            User cur = userService.findByUsername(actionId);
            if (cur != null && cur.getReq().equals("0")) {

               // User owner = userService.findByMessengerid(senderId);
                User owner = userService.findByUsername("1803015");
                String response = owner.getUsername() + " Hall : " + owner.getHall() + " room no : " + owner.getRoom() + " Phone : " + owner.getPhone() + " accepted Your request";

                cur.setReq(owner.getUsername());
                owner.setContribution(owner.getContribution() + 1);
                userService.updateUser(cur);
                userService.updateUser(owner);
                sendTextMessage(cur.getMessengerid(), response);
            } else {
                sendTextMessage(senderId, actionId + " already borrowed someone's bi-cycle");
            }
            return "Done";
        }
